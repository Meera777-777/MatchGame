...
function onCardFlipped(newlyFlippedCard) {
  ...
  if (!doCardsMatch(lastCardFlipped, newlyFlippedCard)) {
    ...
    }, 1000); // give the user a moment to see the second card
    clickAudio.play();
  ...
  // adding a "glow" effect to the matching cards
...
function setUpGame() {
  ...
  // shuffle the image classes
  const shuffledImageClasses = shuffleCardImageClasses();

  //create cards using the shuffled image classes
  const cardObjects = createCards(cardContainer, shuffledImageClasses);

  // add event listeners to cards to handle flipping.
  cardObjects.forEach((card) => {
    card.element.addEventListener("click", () => {
      if (!card.element.classList.contains("flipped")) {
